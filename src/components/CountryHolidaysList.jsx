...
const { FiCalendar, FiInfo, FiClock, FiExternalLink, FiEdit3, FiSave, FiX } = FiIcons;

const CountryHolidaysList = ({ isAdmin }) => {
  const { 
    filteredHolidays, 
    loading, 
    searchTerm, 
  ...
  const handleEditDescription = () => {
    setEditedDescription(countryDescription);
    setIsEditingDescription(true);
  };

  const handleSaveDescription = () => {
    updateCountryDescription(selectedCountry, editedDescription);
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-2xl font-bold text-gray-800">
            {country ? `${country.name} Public Holidays ${selectedYear}` : 'Public Holidays'}
          </h2>
          {isAdmin && (
